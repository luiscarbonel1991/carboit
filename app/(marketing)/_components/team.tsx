...
const team = [
  {
    name: "Luis Carbonel",
    profile: "/static/images/authors/luis.webp",
  ...
  {
    name: "Ernesto Mato",
    profile: "/static/images/authors/ernesto.webp",
  ...
  return (
    <Card className="text-center text-gray-500 dark:text-gray-400 p-4 hover:shadow-lg hover:bg-zinc-100">
      <Image
        ...
        loading={"lazy"}
      />
      <h3 className="mb-1 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{name}</h3>
      <p>{role}</p>
      <ul className="flex justify-center mt-4 space-x-4">
        {Object.entries(socialMediaLinks).map(([key, value]) => {
          if (value) {
            return (
              <li key={key}>
                <Link href={value} className="text-[#39569c] hover:text-gray-900 dark:hover:text-white">
    ...
    <section className="bg-white dark:bg-gray-900">
      <div className="py-8 px-4 mx-auto max-w-screen-xl text-center lg:px-6">
        <div className="mx-auto mb-8 max-w-screen-sm lg:mb-16">
          <h2 className="mb-4 text-4xl font-medium sm:text-5xl text-gray-900 dark:text-white">{teamDic.title}</h2>
          <p className="font-light text-gray-500 sm:text-xl dark:text-gray-400">{teamDic.description}</p>
        </div>
