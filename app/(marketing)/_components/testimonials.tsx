...
const testimonials = [
  {
    ...
    image: "/static/images/marketing/testimonials/taxipremium24h.webp",
  ...
  },
  {
    ...
    image: "/static/images/marketing/testimonials/reyesgroupflorida.webp",
  ...
  },
  {
    ...
    image: "/static/images/marketing/testimonials/dacostaupholstery.webp",
    link: "https://dacostaupholstery.com"
  }
]
const Testimonial = ({ dictionary }: { dictionary: Awaited<ReturnType<typeof getDictionary>>["landing"] }) => {
  ...
  return (
    <section className="flex flex-col items-center overflow-x-hidden">
      ...
      <Carousel
        ...
        className={cn("max-w-7xl mx-auto sm:px-6 lg:px-8 sm:max-w-3xl")}
      >
          ...
          {testimonials.map((item, index) => (
            <CarouselItem key={index}>
              <figure className="grid grid-cols-[auto,1fr] items-center gap-x-4 gap-y-8 sm:grid-cols-12 sm:grid-rows-[1fr,auto,auto,1fr] sm:gap-x-10 lg:gap-x-16 my-32">
                <blockquote className="col-span-2 text-xl/7 text-neutral-600 sm:col-span-7 sm:col-start-6 sm:row-start-2 md:text-balance">
                  <p className="sm:max-w-md">{item.quote}</p>
                </blockquote>
                ...
                <figcaption className="text-sm text-neutral-950 sm:col-span-7 sm:row-start-3 sm:text-base">
                  <Link href={item.link} target={"_blank"} aria-label={`Visit ${item.name} website`}>
                    <span className="font-semibold">{item.name}</span>
                    <span className="hidden font-semibold sm:inline">, </span>
                    <br className="sm:hidden" />
                    <span className="sm:font-semibold underline">{item.position}</span>
                  </Link>
                </figcaption>
              </figure>
            </CarouselItem>
